...
public class VendaDAO {
    ...
    public void cancelarVenda(int vendaId, String motivo) {
        ...
        try (Connection conn = Conexao.getConnection()) {
            ...
            // 🔒 BLOQUEIO
            String status = buscarStatus(vendaId);
    ...
    // ✅ HISTÓRICO DO CLIENTE (CORRIGIDO)
    public List<VendaModel> buscarPorCliente(int clienteId) {

        List<VendaModel> lista = new ArrayList<>();

        String sql = "SELECT id, cliente_id, data, total FROM venda WHERE cliente_id = ? ORDER BY data DESC LIMIT 5";
